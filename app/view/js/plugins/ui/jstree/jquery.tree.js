        ...
        defaults	: {
            ...
            callback	: {
                ...
                ondata		: function(DATA,TREE_OBJ) {
                    return DATA;
                },		// modify data before parsing it
                onparse		: function(STR,TREE_OBJ) {
                    return STR;
                },		// modify string before visualizing it
    ...
    function tree_component () {
        return {
            ...
            init : function(elem, conf) {
                    ...
                    if(this.settings.ui.theme_path === false) {
                        $("script").each(function () { 
                            if(this.src.toString().match(/jquery\.tree.*?js$/)) {
                                _this.settings.ui.theme_path = this.src.toString().replace(/jquery\.tree.*?js$/, "") + "themes/" + _this.settings.ui.theme_name + "/style.css";
                                return false;
                            }
                        });
                    }
                ...
                if(type_icons != "") tree_component.add_sheet({
                    str : type_icons
                });
                ...
                if(tree_component.drag_drop.marker === false) tree_component.drag_drop.marker = $("<div>").attr({
                    id : "jstree-marker"
                }).hide().appendTo("body");
            ...
            close_all : function (obj) {
                ...
                obj = obj ? this.get_node(obj) : this.container;
                // maybe add .andSelf()
                obj.find("li.open").each( function () {
            ...
            moved : function (what, where, how, is_new, is_copy, rb) {
                ...
                if(how == "inside" && this.settings.data.async) {
                    ...
                    if(this.get_node($where).find("> ul > li > a.loading").size() == 1) {
                        setTimeout(function () {
                            _this.moved.apply(_this, [what, where, how, is_new, is_copy]);
                        }, 200);
                        return;
                    }
                }
            ...
            search : function(str, func) {
                ...
                if(this.settings.data.async) {
                    ...
                    nn.parents("li.closed").each( function () {
                        _this.open_branch(this, true);
                    });
    ...
    tree_component.mousemove = function(event) {
        ...
        if(tmp.isdown) {
            ...
            tmp.scroll_time = setTimeout( function() {
                tree2.scroll_check(event.pageX,event.pageY);
            }, 50);
