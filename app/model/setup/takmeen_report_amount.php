...
class ModelSetupTakmeenReportAmount extends HModel {
    ...
    private function getSubQuery() {
        ...
        $sql .=", v.amount, v.created_at, v.created_by_id";
    ...
    public function getMohallahUsers($mohallah_id) {
        $sql = "SELECT u.`user_id`, u.`username`, u.`firstname`, u.`lastname`, u.`ip`";
        $sql .= " FROM `_vaj_form_user` u";
        $sql .= " INNER JOIN `_vaj_form_user_mohallah` um ON um.`user_id` = u.`user_id` AND um.`mohallah_id`='$mohallah_id'";
