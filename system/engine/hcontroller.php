...
class HController extends Controller {
  ...
  public function delete() {
        ...
        if (isset($this->request->post['selected']) && $this->validateDelete()) {
            ...
            $this->db->beginTransaction();
            $this->deleteData($this->request->get['id']);
            $this->db->commit();
            ...
            $this->redirect($this->url->link($this->getAlias(), 'token=' . $this->session->data['token'] . $url, 'SSL'));
//            echo '<div class="success">'.$this->language->get('text_success_delete').'</div>';
//            exit;
        }
    ...
    protected function getForm() {
        ...
        if (!isset($this->request->get[$this->getPrimaryKey()])) {
            $this->data['action'] = $this->url->link($this->getAlias() . '/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        } else {
            $this->data['action'] = $this->url->link($this->getAlias() . '/update', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $this->request->get[$this->getPrimaryKey()] . $url, 'SSL');
        }

        $this->data['cancel'] = $this->url->link($this->getAlias(), 'token=' . $this->session->data['token'] . $url, 'SSL');
    ...
    protected function getBreadCrumbs() {
        ...
        $this->data['breadcrumbs'][] = array(
            'text' => $this->language->get('text_' . $arrBreadCrumbs[0]),
