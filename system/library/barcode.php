...
class BARCODE {
    ...
    function BARCODE($encoding="EAN-13") {
        ...
        $this->_font = dirname($_SERVER["DOCUMENT_ROOT"]) . "/" . "arialbd.ttf";
    ...
    function displayDigit($display_digit = true) {
        $this->display_digit = $display_digit;
    }
    ...
    function _c128Encode($barnumber, $useKeys) {
        ...
        if ($useKeys == 'C') {
            for ($i = 0; $i < strlen($barnumber); $i+=2) {
                ...
                if (is_numeric($val))
                    $sum+=$weight * (int) ($val); elseif ($barnumber == chr(129))
                    $sum+=$weight * 100; elseif ($barnumber == chr(130))
                    $sum+=$weight * 101;
